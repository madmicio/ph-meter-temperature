...
class PhMeterCard extends LitElement {
  ...
  render() {
    ...
    return html`
      ...
      <div class="temprature-container" @click=${() => this._moreinfo(this.config.temperature)}>
    ...
    ${compact === false ? html` 
    <div class="logo" @click=${() => this._moreinfo(this.config.entity)}>
...
<div class="ph-meter-container" @click=${() => this._moreinfo(this.config.entity)}>
      ...
      <div class="item-ph-bar" style="width: ${(stateObj.state * 6.66) - 2}%;">|</div>
      <div class="item-shadow"></div>
    </div>
<div class="ph-meter-states">
...
.grid-item,.grid-item-temp,.grid-item-text-box {
    text-align: center;
    font-weight: 700
}
...
.item-ph-bar,.item-shadow,.item-temp {
    grid-column: 1/15;
    grid-row: 1/2;
