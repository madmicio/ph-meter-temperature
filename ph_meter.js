      ...
      ${compact === false ? html`
        ...
        <div class="ph_name_full" style="width: ${this.config.temp_high || this.config.temp_low || this.config.ph_high || this.config.ph_low ? '80%' : '100%'};">     
          <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" auto size viewBox="0 0 648 512">
            ...
            <text id="Very_Alkaline" data-name="Very Alkaline" class="cls-3" x="414.5" y="391">${this.hass.states[this.config.ph_state].state != "unavailable" ? ph_state : 'no pH data '}</text>
          </svg>
      ...
      `}
          ...
          <div class="ph_compact" >
            <div>${this.config.name ? html`${this.config.name}` : html``} - pH: ${stateObj.state != "unavailable" ? stateObj.state : 'no data '} - ${this.config.ph_state ? html`
              ${this.hass.states[this.config.ph_state].state != "unavailable" ? ph_state : ' '}
            ` : html``}
            </div>
          </div>
      `}
    <!-- ################################################################ badge section ############################################################ -->
      ${this.config.ec || this.config.tds || this.config.salinity ? html`
